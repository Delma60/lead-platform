...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!['LinkedIn', 'X'].includes(platform) || !prompt || prompt.length > 10_000) return NextResponse.json({ error: 'Platform and a prompt of 10,000 characters or fewer are required' }, { status: 400 });
    ...
    const [created] = await db.insert(content).values({ platform: platform as 'LinkedIn' | 'X', draftText: result.draft.trim().slice(0, 20_000), sourcePrompt: prompt, status: 'draft', reviewStatus: 'needs_review' }).returning();
    if (Number.isSafeInteger(Number(body.ideaId))) await db.update(contentIdeas).set({ status: 'drafted', updatedAt: new Date() }).where((await import('drizzle-orm')).eq(contentIdeas.id, Number(body.ideaId)));
