...
export async function PATCH(request: NextRequest, context: Context) {
  try {
    ...
    if (scheduling && parsed.data.scheduledAt && parsed.data.scheduledAt.getTime() <= Date.now()) return NextResponse.json({ error: 'Schedule content for a future time' }, { status: 400 });
