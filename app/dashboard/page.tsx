...
type Analytics = {
  ...
  contentPerformance: { platform: string; posts: number; likes: number; comments: number; reposts: number; clicks: number }[];
  contentPipeline: { needsReview: number; scheduled: number; posted: number };
...
export default function DashboardPage() {
  ...
  return <main className="min-h-screen bg-slate-50 px-5 py-8 text-slate-900 sm:px-8"><div className="mx-auto max-w-7xl">
    ...
    ].map(([label, value, detail], index) => <article key={label} className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><span className={`absolute inset-x-0 top-0 h-1 ${['bg-teal-500','bg-sky-500','bg-violet-500','bg-amber-500'][index]}`}/><p className="text-xs font-bold uppercase tracking-[.12em] text-slate-500">{label}</p><p className="mt-3 text-4xl font-bold tracking-[-.05em] text-[#142524]">{value}</p><p className="mt-2 text-xs font-medium text-slate-400">{detail}</p></article>)}</section>

    <section className="relative mt-6 overflow-hidden rounded-2xl border border-teal-300/10 bg-[#142b29] p-6 text-white shadow-[0_18px_45px_rgba(18,43,41,.16)]"><div aria-hidden="true" className="absolute -right-10 -top-20 size-56 rounded-full bg-teal-400/10 blur-2xl"/><div className="relative flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-widest text-teal-300">Weekly digest</p><p className="mt-2 max-w-3xl text-lg font-medium leading-7">{data.digest.summary}</p></div><div className="flex gap-3"><span className="rounded-xl border border-white/10 bg-white/8 px-3 py-2 text-sm"><strong className="text-teal-300">{data.digest.overdue}</strong> overdue</span><span className="rounded-xl border border-white/10 bg-white/8 px-3 py-2 text-sm"><strong className="text-amber-300">{data.digest.stale}</strong> stale</span></div></div></section>
    ...
    <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="font-bold">Content performance</h2><p className="mt-1 text-sm text-slate-500">Published engagement by platform.</p></div><div className="flex gap-2 text-xs"><span className="badge bg-amber-50 text-amber-800">{data.contentPipeline.needsReview} review</span><span className="badge bg-violet-50 text-violet-700">{data.contentPipeline.scheduled} scheduled</span><span className="badge bg-emerald-50 text-emerald-700">{data.contentPipeline.posted} posted</span></div></div><div className="mt-5 grid gap-4 md:grid-cols-3">{data.contentPerformance.map((item) => <article key={item.platform} className="rounded-xl bg-slate-50 p-4"><div className="flex justify-between"><strong>{item.platform}</strong><span className="text-sm text-slate-500">{item.posts} posts</span></div><p className="mt-3 text-sm text-slate-600">{item.likes} likes · {item.comments} comments</p><p className="mt-1 text-sm text-slate-600">{item.reposts} reposts · {item.clicks} clicks</p></article>)}</div></section>
  </div></main>;
}
